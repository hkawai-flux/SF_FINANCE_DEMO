...
FROM (
    -- 1. 国内株式現物預り明細から抽出
    SELECT ACCOUNT_HK, POSITION_HK, RECORD_SOURCE FROM sbi_staging.stg_bl_int_stock_test
    UNION ALL
    -- 2. 国内株式信用建玉明細から抽出
    SELECT ACCOUNT_HK, POSITION_HK, RECORD_SOURCE FROM sbi_staging.stg_bl_trust_stock_test
) src
WHERE 
...
QUALIFY ROW_NUMBER() OVER (
    PARTITION BY ACCOUNT_HK, POSITION_HK 
