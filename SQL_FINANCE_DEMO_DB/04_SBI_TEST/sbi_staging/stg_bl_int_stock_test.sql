...
-- 国内株式現物預り明細テスト用テーブルの作成
CREATE OR REPLACE TABLE sbi_staging.stg_bl_int_stock_test (
    ...
    other_val VARIANT COMMENT 'その他属性（ディーラー番号、NISA区分等）',
    
    PRIMARY KEY (position_hk)
) COMMENT = '国内株式現物預り明細（テスト用ステージングテーブル）';

INSERT INTO sbi_staging.stg_bl_int_stock_test
SELECT 
    ...
    'TEST_GEN_BL_INT_STOCK' as record_source,
    OBJECT_CONSTRUCT(
        'DEALER_NO', 'D' || LPAD(UNIFORM(1, 999, RANDOM()), 3, '0'),
        'JNISA_ACCOUNT_KBN', decode(UNIFORM(1, 3, RANDOM()), 1, '1', 2, '2', '0')
    ) as other_val
FROM (
    SELECT 
        ...
        -- br_comp_code(5桁) + br_n_o_ind(1桁)
        LPAD(UNIFORM(1000, 9999, RANDOM()), 5, '0') || '0' as brand_cd,
        UNIFORM(500, 20000, RANDOM()) as price_mx_revise_value, -- 修正評価単価
        UNIFORM(1, 100, RANDOM()) * 100 as exec_base_balance_t1, -- 残高
        (price_mx_revise_value * 0.95)::INT as acquire_unit_price, -- 取得単価（仮）
